...
export const StatsSection: React.FC<StatsSectionProps> = ({ teachers, currentLang }) => {
  ...
  const stats = useMemo(() => {
    ...
    const healthy = reviewedTeachers.filter((tch) => ratingTone(tch.averageRating) === 'success').length;
    const mixed = reviewedTeachers.filter((tch) => ratingTone(tch.averageRating) === 'warning').length;
    const flagged = reviewedTeachers.filter((tch) => ratingTone(tch.averageRating) === 'danger').length;
