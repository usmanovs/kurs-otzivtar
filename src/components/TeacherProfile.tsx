...
// The pieces of a teacher profile shared by the modal (opened from the
// homepage) and the standalone /teacher/:id page. Only the surrounding chrome
// differs — the overlay, scroll container and close button live in
// TeacherDetailModal; the breadcrumb and window-level scrolling in TeacherPage.

export function formatDateTime(iso: string): string {
  const d = new Date(iso);
  if (Number.isNaN(d.getTime())) return iso;
  // Fixed UTC+6 (Kyrgyzstan has no DST) rather than the runtime's zone, so the
  // prerendered HTML and the browser render the same string.
  const k = new Date(d.getTime() + 6 * 3600 * 1000);
...
const ReviewTrustBadge: React.FC<{ review: Review; currentLang: SupportedLang }> = ({
  ...
  const Icon = tier === 'proof' ? ShieldCheck : tier === 'imported' ? Youtube : User;
...
export function useTeacherShare(teacher: Teacher, currentLang: SupportedLang) {
  const t = TRANSLATIONS[currentLang];
  const [copied, setCopied] = useState(false);
  const share = async () => {
    const shareUrl = `${SITE_URL}/teacher/${teacher.id}`;
    const shareData = {
...
export const ShareButton: React.FC<{ teacher: Teacher; currentLang: SupportedLang }> = ({ teacher, currentLang }) => {
  const t = TRANSLATIONS[currentLang];
  const { copied, share } = useTeacherShare(teacher, currentLang);
  return (
    <button
      type="button"
...
/** TikTok / website icons — Instagram and YouTube sit beside the name instead. */
export const TeacherSocialLinks: React.FC<{ teacher: Teacher }> = ({ teacher }) => {
  const socialLinks = [
    { url: teacher.tiktokUrl, label: 'TikTok', Icon: TikTokIcon, hover: 'hover:text-white' },
    { url: teacher.websiteUrl, label: 'Website', Icon: Globe, hover: 'hover:text-white' },
  ].filter((s) => s.url);
  if (socialLinks.length === 0) return null;
  return (
    <div className="flex items-center gap-0.5 pr-1 mr-1 border-r border-white/15">
      {socialLinks.map(({ url, label, Icon, hover }) => (
        <a
          key={label}
          href={url}
          target="_blank"
          ...
          aria-label={label}
          className={`w-9 h-9 flex items-center justify-center rounded-full text-white/70 transition-colors ${hover}`}
        >
...
/** Photo, name (as the page's heading), socials, tags and the rating pill. */
export const TeacherHeaderIdentity: React.FC<{
  teacher: Teacher;
  currentLang: SupportedLang;
  as?: 'h1' | 'h2';
}> = ({ teacher, currentLang, as: Heading = 'h2' }) => {
  const t = TRANSLATIONS[currentLang];
          ...
          <div className="flex items-start gap-3 sm:gap-3.5 min-w-0">
            ...
            <div className="min-w-0">
              <div className="flex items-center gap-1.5 min-w-0">
                <Heading className="text-lg sm:text-xl font-bold leading-tight tracking-tight break-words">
                  {teacher.name}
                </Heading>
                {teacher.instagramUrl && (
                  <a
                    ...
                    className="shrink-0 text-white/70 hover:text-pink-400 transition-colors"
                  >
...
interface TeacherProfileBodyProps {
  ...
  isAdmin: boolean;
  /** 'modal' keeps the claim-profile bar sticky inside the scroller. */
  variant: 'modal' | 'page';
  onOpenAddReview: () => void;
  onVoteReview: (teacherId: string, reviewId: string, type: 'helpful' | 'unhelpful') => void;
}

export const TeacherProfileBody: React.FC<TeacherProfileBodyProps> = ({
  teacher,
  currentLang,
  highlightReviewId,
  isAdmin,
  ...
  // Pros/cons pills collapse past a handful per card, keyed by review id.
  ...
  return (
    <>
              ...
              {filteredReviews.length === 0 ? (
                ...
                filteredReviews.map((review) => {
                  ...
                  >
                      ...
                      <div className="flex items-start gap-2.5 min-w-0">
                        ...
                        <div className="min-w-0">
                          ...
                          {/* Wraps as items rather than one text run, so a long
                              ...
                              date off the card at 375px. Each bullet lives in
                              the same span as the text that follows it, so a
                              wrap point can never strand a lone "•" alone at
                              the start of a line. */}
                          <div className="flex flex-wrap items-center gap-x-1.5 gap-y-1 mt-0.5 text-2xs text-slate-500">
                            ...
                            {review.cohortYear && (
                              <span className="whitespace-nowrap">• {review.cohortYear}</span>
                            )}
                            {review.durationMonths && (
                              <span className="whitespace-nowrap">• {review.durationMonths} ай окуган</span>
                            )}
                    ...
                    {/* The title is never user-authored — it's one of two
                        fixed strings picked by the recommend toggle at
                        submission — so showing it as its own headline just
                        repeated the recommend pill below in bigger type. The
                        reviewer's actual words get the emphasis instead. */}
                    <p className="text-sm sm:text-base text-slate-800 leading-relaxed whitespace-pre-line mt-2.5">
                      {review.fullReview}
                    </p>

                    {/* Verdict row: always visible, never collapsed — these
                        are trust signals, not attribute chatter. */}
                    <div className="flex flex-wrap items-center gap-1.5 mt-3">
                      ...
                      {review.hasJobScamReport && (
                        <>
                          {/* The full claim reads as a paragraph at 375px wide
                              inside a pill — a short one-liner keeps it a
                              single line without losing the warning. */}
                          <span className="sm:hidden inline-flex items-center gap-1 rounded-full px-2.5 py-1.5 text-xs font-medium border bg-red-50 text-red-700 border-red-200 whitespace-nowrap">
                            <span aria-hidden="true">⚠️</span>
                            {t.detailModal.scamWarningShort}
                          </span>
                          <span className="hidden sm:inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-xs font-semibold border bg-red-50 text-red-700 border-red-200">
                    ...
                    {/* Attribute pills: pros then cons, capped so a review with
                        six or seven of them doesn't turn into its own wall of
                        chips — expand on demand instead. */}
                    {(() => {
                      const tags = [
                        ...review.pros.map((text) => ({ text, kind: 'pro' as const })),
                        ...review.cons.map((text) => ({ text, kind: 'con' as const })),
                      ];
                      if (tags.length === 0) return null;
                      const VISIBLE_LIMIT = 4;
                      const isExpanded = !!expandedTags[review.id];
                      const visibleTags = isExpanded ? tags : tags.slice(0, VISIBLE_LIMIT);
                      ...
                      return (
                        <div className="flex flex-wrap items-center gap-1.5 mt-1.5">
                          ...
                          {hiddenCount > 0 && (
                            <button
                              type="button"
                              onClick={() => setExpandedTags((prev) => ({ ...prev, [review.id]: true }))}
                              className={`${pill} bg-slate-50 text-slate-500 border-slate-200 hover:bg-slate-100 cursor-pointer`}
                            >
                              {t.detailModal.moreTagsBtn.replace('{n}', String(hiddenCount))}
                            </button>
                          )}
                          {isExpanded && tags.length > VISIBLE_LIMIT && (
                            <button
                              type="button"
                              onClick={() =>
                                setExpandedTags((prev) => ({ ...prev, [review.id]: false }))
                              }
                              className={`${pill} bg-slate-50 text-slate-500 border-slate-200 hover:bg-slate-100 cursor-pointer`}
                            >
                              {t.detailModal.lessTagsBtn}
                            </button>
                          )}
                        </div>
                      );
                    ...
                    <div className="flex items-center justify-between gap-2 text-xs text-slate-500 pt-2.5 mt-2.5 border-t border-slate-100">
                      ...
                      {!review.proofVerified && (
                        <button
                          ...
                          className="inline-flex items-center gap-1 rounded-full border border-indigo-100 bg-indigo-50/70 px-2.5 py-1 text-[11px] font-semibold text-indigo-600 hover:bg-indigo-100 hover:border-indigo-200 transition-colors cursor-pointer shrink-0"
                        >
          ...
          {responses.length === 0 && variant === 'page' && (
            <div className="flex justify-center sm:justify-end border-t border-slate-200 pt-4">
              <button
                type="button"
                id="page-claim-profile-btn"
                onClick={() => setIsRespondOpen(true)}
                className="inline-flex w-full sm:w-auto min-w-0 items-center justify-center gap-1.5 rounded-xl sm:rounded-full border border-slate-300 bg-white px-4 py-3 sm:py-2 text-sm sm:text-xs font-medium sm:font-semibold text-slate-700 sm:shadow-xs transition-colors active:bg-slate-100 sm:hover:bg-slate-50 cursor-pointer shrink-0 text-center"
              >
                <MessageSquareReply className="w-4 h-4 sm:w-3.5 sm:h-3.5 shrink-0" />
          ...
          {/* Sticky inside the scroll container, not a fixed bar outside it:
              ...
              needed to clear it. Social links live in the header now, next to
              Share/Close, so this bar is the claim-profile CTA alone. */}
          {responses.length === 0 && variant === 'modal' && (
            <div
              className="sticky bottom-0 left-0 right-0 z-30 -mx-5 sm:-mx-7 mt-6 flex justify-center sm:justify-end border-t border-slate-200 bg-white/95 backdrop-blur-sm px-4 py-3 shadow-[0_-4px_12px_rgba(0,0,0,0.05)]"
      ...
      {/* display:contents so the wrapper adds no box — and, crucially, no
          space-y margin — to the fixed overlays inside it. */}
