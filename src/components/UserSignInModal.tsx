...
import { sendReviewEmailCode, signInWithGoogle, verifySignInCode } from '../lib/auth';
...
export const UserSignInModal: React.FC<UserSignInModalProps> = ({
  ...
  const handleGoogleSignIn = async () => {
    ...
    setError('');
    try {
      // Redirects the page away on success, so nothing to do after this
      // resolves — it only returns early if Google itself couldn't be reached.
      await signInWithGoogle();
    } catch {
      setError(t.userAuth.googleSignInFailed);
          ...
          {step === 'email' ? (
            <div className="space-y-4">
              <button
                ...
                className="w-full inline-flex items-center justify-center gap-2.5 px-5 py-2.5 bg-white hover:bg-slate-50 disabled:opacity-60 border border-slate-300 text-slate-700 font-semibold text-sm rounded-full transition-colors shadow-xs cursor-pointer"
              >
                <GoogleIcon className="w-4 h-4 shrink-0" />
                <span>{googleLoading ? t.userAuth.sending : t.userAuth.continueWithGoogle}</span>
              </button>

              <div className="flex items-center gap-3">
                <div className="h-px flex-1 bg-slate-200" />
                <span className="text-2xs font-medium text-slate-400">{t.userAuth.orDivider}</span>
                <div className="h-px flex-1 bg-slate-200" />
              </div>
